...
template <int N>
void dropFront(int32_t arr[N], int32_t *res) {
...
struct StridedMemRefType {
  ...
  T &operator[](Range &&indices) {
    ...
    for (int dim = N - 1; dim >= 0; --dim) {
      int32_t currentIndex = *(indices.begin() + dim);
  ...
  StridedMemRefType<T, N - 1> operator[](int32_t idx) {
...
public:
  ...
  const std::array<int32_t, Rank> &getIndices() { return indices; }
...
private:
  ...
  std::array<int32_t, Rank> indices = {};
...
public:
  ...
  StridedMemrefIterator(StridedMemRefType<T, 0> &descriptor, int32_t offset = 0)
  ...
  const std::array<int32_t, 0> &getIndices() {
    // Since this is a 0-array of indices we can keep a single global const
    // copy.
    static const std::array<int32_t, 0> indices = {};
    return indices;
  }
...
public:
  ...
  // This operator[] is extremely slow and only for sugaring purposes.
  DynamicMemRefType<T> operator[](int32_t idx) {
...
public:
  ...
  DynamicMemRefIterator(DynamicMemRefType<T> &descriptor, int32_t offset = 0)
      : offset(offset), descriptor(&descriptor) {
  ...
  const std::vector<int32_t> &getIndices() { return indices; }
