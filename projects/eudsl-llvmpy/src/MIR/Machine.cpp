...
void populate_mir(nb::module_ &m) {
  ...
  // stub named `name` (a MachineFunction must attach to one) with a trivial
  // body so it is a *definition* -- codegen only processes defined functions,
  // so emit_object would otherwise skip it. Hold the MachineFunction in a
