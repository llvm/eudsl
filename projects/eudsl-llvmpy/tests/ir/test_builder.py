...
def test_build_conditional_with_phi():
    with llvm.ir.Context() as ctx:
        ...
        fn = llvm.ir.Function.create(llvm.types.function(i32, [i1]), "sel", mod)
...
def test_icmp():
    with llvm.ir.Context() as ctx:
        ...
        assert isinstance(result, llvm.ir.ICmpInst)
        del b, fn, bb, mod
    assert_no_leaks()
...
def test_fcmp():
    with llvm.ir.Context() as ctx:
        ...
        assert isinstance(result, llvm.ir.FCmpInst)
        del b, fn, bb, mod
    assert_no_leaks()
...
def test_gep():
    with llvm.ir.Context() as ctx:
        ...
        assert isinstance(result, llvm.ir.GetElementPtrInst)
        del b, fn, bb, mod
    assert_no_leaks()
...
def test_call():
    with llvm.ir.Context() as ctx:
        ...
        assert isinstance(result, llvm.ir.CallInst)
...
def test_i32_const_and_i64_const():
    with llvm.ir.Context() as ctx:
        ...
        with InsertPoint(bb, builder=b):
            ...
            c64 = b.i64_const(100)
            b.ret(c64)
        assert isinstance(c32, llvm.ir.ConstantInt)
        assert isinstance(c64, llvm.ir.ConstantInt)
        printed = str(mod)
