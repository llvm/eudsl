...
def test_linkage_weak():
    with llvm.ir.Context() as ctx:
        mod = llvm.ir.Module("m", ctx)
        f = _fn(ctx, mod)
        f.linkage = llvm.ir.Linkage.WEAK
...
def test_linkage_linkonce_odr():
    with llvm.ir.Context() as ctx:
        mod = llvm.ir.Module("m", ctx)
        f = _fn(ctx, mod)
        f.linkage = llvm.ir.Linkage.LINKONCE_ODR
...
def test_calling_conv_cold():
    with llvm.ir.Context() as ctx:
        mod = llvm.ir.Module("m", ctx)
        f = _fn(ctx, mod)
        f.calling_conv = llvm.ir.CallingConv.COLD
        assert f.calling_conv == llvm.ir.CallingConv.COLD
...
def test_calling_conv_c():
    with llvm.ir.Context() as ctx:
        mod = llvm.ir.Module("m", ctx)
        f = _fn(ctx, mod)
        f.calling_conv = llvm.ir.CallingConv.C
        assert f.calling_conv == llvm.ir.CallingConv.C
        printed = str(mod)
...
def test_visibility_protected():
    with llvm.ir.Context() as ctx:
        mod = llvm.ir.Module("m", ctx)
        f = _fn(ctx, mod)
        f.visibility = llvm.ir.Visibility.PROTECTED
        assert f.visibility == llvm.ir.Visibility.PROTECTED
...
def test_visibility_default():
    with llvm.ir.Context() as ctx:
        mod = llvm.ir.Module("m", ctx)
        f = _fn(ctx, mod)
        f.visibility = llvm.ir.Visibility.DEFAULT
