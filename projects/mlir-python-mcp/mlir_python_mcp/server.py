...
_TOOLS: list[types.Tool] = [
    ...
    types.Tool(
        ...
        description=(
            ...
            "Dialect submodules are not pre-imported, so import the ones you need first, "
            "e.g. 'from mlir.dialects import arith, func, scf, linalg'. "
            "Until that import runs, attribute access such as dialects.arith raises AttributeError. "
            ...
            "Discovery: use help(<class>) for docs/signatures (e.g. help(arith.AddIOp), "
            "help(Pipeline.canonicalize), help(ir.IntegerType)), "
            "dir(arith) to list available ops, "
            "and dir(Pipeline) to list all passes. "
            "Variables persist between calls. Use generated builders for types/attrs/ops "
            "(e.g. ir.IntegerType.get_signless(32), arith.AddIOp(lhs, rhs)). "
            "Note: PascalCase op classes (arith.AddIOp) return OpView objects — use .result to get the Value. "
            "snake_case helpers (arith.addi) return Values directly. "
